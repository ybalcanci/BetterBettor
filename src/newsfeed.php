...
<div class="container" style="margin-top:80px;">
    ...
    <div id="navigation" >
      ...
      <!-- Coupons -->
      <div class="panel container-fluid col-md-7" style="margin-right: 15px">
          ...
          $sql = "SELECT * FROM Post where tid =  " . $_SESSION['tid'] . " or tid in (SELECT followed_tid FROM Follow WHERE follower_tid = " . $_SESSION['tid'] . ") ORDER BY post_id DESC ;" ;
    	  $posts = mysqli_query($conn, $sql);
          while($post = mysqli_fetch_assoc($posts)) {


          $sql = "SELECT * FROM User where tid = ". $post['tid'] . ";" ;
          $user = mysqli_fetch_assoc(mysqli_query($conn, $sql));

          echo "<a href=\"userProfile.php?tid=". $user['tid']."\">". $user["name"] ." ". $user["surname"]. "</a>";

			if($post['description'] == ""){
				$sql = "SELECT * FROM Post, Post_Coupon WHERE
              		Post.post_id = Post_Coupon.post_id AND (tid = ". $_SESSION['tid'] . " or tid in ( SELECT followed_tid FROM Follow WHERE follower_tid = " . $_SESSION['tid'] . ")) AND Post.post_id = ".$post['post_id'].";";
			...
			<form action='report.php?post_id=".$post['post_id']."' method='post' style='float: left;'>
	...
	<div class="search-container col-md-0" >
    		<form action="search.php" method="POST" >
      			<input type="text" placeholder="Search.." pattern=".{3,}" required title="3 characters minimum" name="search">
