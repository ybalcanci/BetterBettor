    ...
    <div id="navigation" >
      ...
      <div class="panel container-fluid col-md-7" >
        <?php
  				...
  				echo "
  				<h3 class=\"page-header\" style=\"font-weight: bold;\"> Popular Members </h3>
  		    	<table class=\"table table-bordered\" style=\"border:solid; margin-bottom: 10px\">
  		    	<thead>
  		      <tr>
  				    <th>Name Surname</th>
            ...
            $sql = "select tid, name, surname, cnt from User natural join (select count(follower_tid) as cnt, followed_tid as tid from Follow group by followed_tid) as T order by cnt DESC, surname ASC, name ASC;";
            ...
            while($follower = mysqli_fetch_assoc($followers) ) {
				...
				//$i = $i + 1;
              $sql = "SELECT * from User where tid =  \"" . $follower['tid'] . "\";";
              $fullname =  mysqli_fetch_assoc(mysqli_query($conn, $sql));
              echo
              "<tbody style='cursor: pointer;' onclick='document.cookie = \"temptid=".$fullname["tid"]."\";'>
                <tr>
                  <td>
